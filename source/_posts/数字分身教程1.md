---
title: 🚀 保姆级教程：用 RTX 5090 + SDXL 打造你的“迪士尼风格”数字分身
date: 2025-12-20
tags: [AI绘画, Stable Diffusion, ComfyUI, LoRA, 实战教程]
excerpt: 只有显卡不会用？本文手把手教你利用 AutoDL 高算力，从零开始训练个人专属 LoRA 模型，并配合 ComfyUI 双 LoRA 串联工作流，完美复刻皮克斯/迪士尼风格的 3D 数字分身。代码可复制，逻辑通俗易懂，亲测跑通！
category: AI实战
---
...
1.  **准备照片**： 挑选 20-30 张你的半身照/大头照。
    * ✅ **要**： 表情自然、光线充足、多角度。
    * ❌ **不要**： 即使是“神图”但画质模糊、脸部被遮挡。
2.  **上传路径**： 将照片上传到 `/root/autodl-tmp/train_data/10_my_face`。
    * *注意：`10_` 代表每张图训练 10 次，这是经验值。*
3.  **清洗标签 (关键！)**：使用打标工具生成 `.txt` 描述文件后，务必手动检查。
...
在 `sd-scripts` 目录下新建 `config.toml`，填入以下“特调配方”：
...
为了达到极致的迪士尼效果，我们需要一个“风格滤镜”来配合你的“面部模型”。
...
### 3. 搭建“双 LoRA 串联”工作流 (核心秘籍)

不要简单地加载模型，要像 **“穿糖葫芦”** 一样串联节点：
...
* **采样器 (KSampler)**: 步数 6-8 (Turbo模式)，CFG 1.5-2.0。
...
> **正向 (Positive):**
> `(Disney 3D style:1.4), (frozen style:1.2), qt_frozen man, male focus, smiling, suit...`

> **负向 (Negative):**
> `(photorealistic:1.5), realism, skin texture` (严防死守写实感)

## 🏆 总结

这一套 **“DreamShaper底模 + 强 Style LoRA + 弱 Face LoRA”** 的组合拳，是目前生成风格化数字分身的最优解。

* **为什么要降权？** 因为你的模型是真人照片练的，权重太高会带入“真人感”，破坏“卡通感”。
* **为什么要串联？** 串联能让模型先学会画风，再学会长相，逻辑更顺畅。

现在，去点击那个 **Queue Prompt** 按钮吧，见证你在元宇宙里的第一次亮相！
